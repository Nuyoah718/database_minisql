...
uint32_t Schema::GetSerializedSize() const
{
  ...
  return size + 2 * sizeof(uint32_t);    //返回计算得到的序列化后的总字节数（要加上SCHEMA_MAGIC_NUM和columns_长度两个uint32_t类型的字节数）
}
...
uint32_t Schema::DeserializeFrom(char *buf, Schema *&schema, MemHeap *heap) 
{
  ...
  {
    Column *col;
    ofs += Column::DeserializeFrom(buf + ofs, col, heap);    //反序列化出一个Column对象
    columns.push_back(col);    //将反序列化出的Column对象添加到columns向量中
  }
  schema = ALLOC_P(heap, Schema)(columns);    //在heap中分配内存，创建一个新的Schema对象，并将columns向量传递给它
  
  return ofs;    //返回反序列化后的字节数
}
